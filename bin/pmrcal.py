...
def process_snp_data(oripath,snppath):
    ...
    def process_distance_range(df2, cut, lower, upper):
        ...
        p = data.select([((data[:, list_ur[t][0]] == data[:, list_ur[t][1]]) |
                          (data[:, list_ur[t][0]] == '-') |
                          (data[:, list_ur[t][1]] == '-')).alias(f'{t}') for t in range(len(list_ur))]).sum_horizontal().to_numpy()
        l = len(list_ur)
        return p, l, mdist
    ...
    def save_results(list_r, list_gr, output_file):
        ...
        with open(output_file, 'w') as f:
            f.write('\n'.join([str(x) for x in res['r'].values]))

    save_results(list1, list1r, f'{snppath}.fa.pmr.30.2.txt')
    save_results(list5, list5r, f'{snppath}.fa.pmr.all.2.txt')
...
if __name__ == "__main__":
    snppath = sys.argv[2]
    oripath = sys.argv[1]
    process_snp_data(oripath,snppath)